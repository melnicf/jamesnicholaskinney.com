...
export const ARTICLES_QUERY = `*[_type == "article" && contentState == "published" && defined(slug.current)] | order(defined(mainImage.asset) desc, publishedAt desc) [0...20] {
  _id,
  title,
  "slug": slug.current,
  excerpt,
...
export const ARTICLES_BY_CATEGORY_QUERY = `*[_type == "article" && contentState == "published" && category->slug.current == $categorySlug && defined(slug.current)] | order(defined(mainImage.asset) desc, publishedAt desc) {
  _id,
  title,
  "slug": slug.current,
  excerpt,
...
export const THIS_WEEK_ARTICLES_QUERY = `*[_type == "article" && contentState == "published" && defined(slug.current) && publishedAt >= $weekAgo] | order(defined(mainImage.asset) desc, publishedAt desc) [0...10] {
  _id,
  title,
  "slug": slug.current,
  excerpt,
...
export const ALL_EVENTS_QUERY = `*[_type == "event" && defined(slug.current)] | order(eventDate asc) {
  _id,
  title,
  "slug": slug.current,
  eventDate,
